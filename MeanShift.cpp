	...
	if (typeXL==1)
	{
		...
		if (len>0)
		{
			for (int j=0;j<len;j++)
			{
				...
				putText(sequence2,"small",
					Point(selectionLY1[j].x+int(0.3*selectionLY1[j].width),selectionLY1[j].y-5),
					CV_FONT_HERSHEY_COMPLEX,0.7,Scalar(0),1);
	...
	else if (typeXL==2)
	{
		...
		if (len>0)
		{
			for (int j=0;j<len;j++)
			{
				...
				putText(sequence2,"middle",
					Point(selectionLY2[j].x+int(0.3*selectionLY2[j].width),selectionLY2[j].y-5),
					CV_FONT_HERSHEY_COMPLEX,0.7,Scalar(0),1);
			}
		}
		return selectionLY2;
	...
	else if(typeXL==3)
	{
		...
		if (len>0)
		{
			for (int j=0;j<len;j++)
			{
				...
				putText(sequence2,"big",
					Point(selectionLY3[j].x+int(0.3*selectionLY3[j].width),selectionLY3[j].y-5),
					CV_FONT_HERSHEY_COMPLEX,0.7,Scalar(0),1);
			}
		}
		return selectionLY3;
